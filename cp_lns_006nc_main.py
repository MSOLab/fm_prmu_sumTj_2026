...
MAIN_METADATA_FILENAME = "cp_lns_006nc_metadata_20260607.yaml"
...
def determine_run_mode_and_base_dir(
    ...
    # Resolve the post-process target dir, either from an explicit analysis
    # path or from a timestamp under the scenarios output dir.
    analysis_path = config.get_analysis_dir_path()
    if analysis_path is not None:
        _target_path = analysis_path
        _timestamp = analysis_path.name
    elif config.analysis_timestamp:
        _target_path = config.output_dir_scenarios / config.analysis_timestamp
