...
from mbls.cpsat.cp_model_with_flexible_interval import CpModelWithFlexibleInterval
...
class CpCpsatPosition(CpModelWithFlexibleInterval):
    ...
    def define_variables(self) -> None:
        ...
        for i in i_list:
            ...
            p_max_i = max(p_set)
            for k in j_list:
                self.define_flexible_interval_var(
                    name=(i, k),
                    p_lb=p_min_i,
                    p_ub=p_max_i,
                    start_lb=self.stage_start_time_lb[i],
                    start_ub=self.stage_end_time_ub[i] - p_min_i,
                )
    ...
    def define_constraints(self) -> None:
        ...
        for i in i_list:
            self.add_no_overlap([self.var_op_intvl[i, k] for k in j_list])
            for k in j_list:
                if k != self.j_first:
